...
def storelist(slist, name):
	list_storage = open(name, "w")
	for file in slist:
		list_storage.write("{}\n" .format(file))
	list_storage.close()

def loadlist(name):
	list_load = open(name, "r")
	llist = list_load.readlines()
	for fi in range(len(llist)):
		end = len(llist[fi]) - 1
		while llist[fi][end]=='\n' or llist[fi][end]=='\r': end -= 1
		llist[fi] = llist[fi][:end+1]
	list_load.close()
	return llist

def dictionary_load(name):
	dictionary = {}
	strload = list_load(name)
