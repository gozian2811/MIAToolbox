...
class CTViewer:
	...
	def __call__(self, event):
		...
		if event.name == 'scroll_event':
			if event.button == 'up':
				self.slicenum = min(self.slicenum+1, self.maxslice-1)
			elif event.button == 'down':
				self.slicenum = max(self.slicenum-1, 0)
			...
			if event.key == 'up':
				...
				self.slicenum = min(self.slicenum + 50, self.maxslice - 1)
			elif event.key == 'pagedown':
				self.slicenum = max(self.slicenum - 50, 0)
		'''
...
def view_coordinates(volume, candidate_coords = [], window_size = 40, reverse = True, slicewise = False, show = True, box_color = 500):
	window_size = np.round(window_size).astype(int)
	half_window = np.int_(window_size/2)
	...
	for cc in range(len(candidate_coords)):
		...
		if window_size.ndim>1:
			wsize = window_size[cc]
			hsize = half_window[cc]
		else:
			wsize = window_size
			hsize = half_window
		bottombound = coord - hsize
		topbound = coord + wsize - hsize
